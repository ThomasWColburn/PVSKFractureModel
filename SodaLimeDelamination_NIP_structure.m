...
% PVSK on Soda Lime Glass, N-I-P structure %
...
% [0.75 0.75 0.75] = Ag RGB code (screen-print)
...
sigma_Ag_screen = (a_Ag_screen - a_SodaLimeGlass)*delta_T_Ag_screen*E_Ag_screen;
...
% Solve for Gss
G_AgSpiro = sigma_Ag_screen^2*h_Ag/E_Ag_screen - (P^2/(B^2*E_Ag_screen*h_Ag) ...
    + E_Ag_screen*h_Ag^3*k^2/12);

% Solve for hc at Gc from literature
% Gc for PCBM is used here because Gc_Spiro < Gc_Ag_screen
hc_AgScreen_AgSpiro = vpasolve(G_AgSpiro == Gc_Spiro, h_Ag, [0,inf]);
hc_AgScreen_AgSpiro = double(hc_AgScreen_AgSpiro);

if length(hc_AgScreen_AgSpiro) > 1
    smallest_val = hc_AgScreen_AgSpiro(1);
    for j = 1:length(hc_AgScreen_AgSpiro)
        if hc_AgScreen_AgSpiro(j) < smallest_val
            smallest_val = hc_AgScreen_AgSpiro(j);
        end
    end
    hc_AgScreen_AgSpiro = smallest_val;
end

disp("hc_{Ag} screen for Ag-Spiro interface delamination: ")
disp(hc_AgScreen_AgSpiro);


figure;
x = [hc_AuEvap_AuSpiro*10^9 hc_AgScreen_AgSpiro*10^9];
b = bar(x, 'FaceColor', 'flat');
b.CData(1,:) = [0.980 0.866 0.505];
b.CData(2,:) = [0.75 0.75 0.75];
xt = get(gca, 'XTick');
set(gca, 'XTick', xt, 'XTickLabel', {'Au evap', 'Ag screen'})
% set(gca,'YScale','log')
...
% Section 3.1: SnOx-PVSK delamination, varying h_PVSK, h_Spiro, h_Au.
% screen Ag
% Vary h_Au
% with PVSK RSPP
syms h_Au
...
disp("hc{Ag} screen for SnOx-PVSKRSPP interface delamination: ")
...
% Set the "default" heights of PVSK and Au evap
h_PVSK = 400e-9;
h_Au_evap = 150e-9;
...
% Find the minimum value for the electrode (both screen-Au and evap-Au)
hc_Ag_screen_PVSKRSPP_values = [hc_AgScreen_AgSpiro; hc_AgScreen_SpiroPVSK; ...
...
hc_Ag_screen_PVSKhotcast_values = [hc_AgScreen_AgSpiro; hc_AgScreen_SpiroPVSK;...
...
hc_Ag_screen_PVSKspincoat_values = [hc_AgScreen_AgSpiro; hc_AgScreen_SpiroPVSK;...
...
b(3).CData(2,:) = [0.192 0.192 0.447]; % group 1 3rd bar
...
figure;
x = [hc_Au_evap_PVSKRSPP*10^9 hc_Spiro_evap_PVSKRSPP*10^9 hc_PVSKRSPP_evap*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9; ...
    hc_Au_evap_PVSKhotcast*10^9 hc_Spiro_evap_PVSKhotcast*10^9 hc_PVSKhotcast_evap*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9;...
    hc_Au_evap_PVSKspincoat*10^9 hc_Spiro_evap_PVSKspincoat*10^9 hc_PVSKspincoat_evap*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9];
b = bar(x, 'FaceColor', 'flat');
xt = get(gca, 'XTick');
...
figure;
x = [hc_Ag_screen_PVSKRSPP*10^9 hc_Spiro_screen_PVSKRSPP*10^9 hc_PVSKRSPP_screen*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9; ...
    hc_Ag_screen_PVSKhotcast*10^9 hc_Spiro_screen_PVSKhotcast*10^9 hc_PVSKhotcast_screen*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9;...
    hc_Ag_screen_PVSKspincoat*10^9 hc_Spiro_screen_PVSKspincoat*10^9 hc_PVSKspincoat_screen*10^9 hc_SnOx_buckle_sodaLimeGlass*10^9 hc_ITOglass_buckle_sodaLimeGlass*10^9];
b = bar(x, 'FaceColor', 'flat');
xt = get(gca, 'XTick');
set(gca, 'XTick', xt, 'XTickLabel', {'RSPP PVSK', 'Hotcast PVSK', 'Spincoat PVSK'})
set(gca,'YScale','log')
ylabel('Critical Debond Thickness h_c, (nm)')
title("Soda lime glass substrate, NIP, screen-printed Ag", 'FontWeight','Normal')
box on;
set(gca, 'FontSize', 14, 'FontName', 'Arial');
% Determine the y-axis limits to use in the bar plot.
y_lowerlimit = (min(min(x))+1)/10;
