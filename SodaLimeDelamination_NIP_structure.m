...
% [1 1 0] = Au RGB code
% [0.5 0.5 0.5] = Ag RGB code (screen-print)
% [0 0.4470 0.7410] = Spiro RGB code
% [0.9290 0.6940 0.1250] = SnOx RGB code
...
% Define P symbolically to plug into equation
h_summation = h_Au;
P = (k*B/(6*h_summation))*(E_Ag_screen*h_Au^3);

% Solve for Gss
G_AuSpiro = sigma_Ag_screen^2*h_Au/E_Ag_screen - (P^2/(B^2*E_Ag_screen*h_Au) ...
    + E_Ag_screen*h_Au^3*k^2/12);
...
% Define P symbolically to plug into equation.
h_summation = h_Au + h_Spiro;
P = (k*B/(6*h_summation))*(E_Ag_screen*h_Au^3 + E_Spiro*h_Spiro^3);

% Solve for Gss
G_SpiroPVSK = sigma_Ag_screen^2*h_Au/E_Ag_screen + sigma_Spiro^2*h_Spiro/E_Spiro ...
...
hc_Ag_screen_PVSKRSPP_values = [hc_AuScreen_AuSpiro; hc_AuScreen_SpiroPVSK; ...
...
hc_Ag_screen_PVSKhotcast_values = [hc_AuScreen_AuSpiro; hc_AuScreen_SpiroPVSK;...
...
hc_Ag_screen_PVSKspincoat_values = [hc_AuScreen_AuSpiro; hc_AuScreen_SpiroPVSK;...
...
b(1).CData(1,:) = [1 1 0]; % group 1 1st bar
b(1).CData(2,:) = [1 1 0]; % group 2 1st bar
b(1).CData(3,:) = [1 1 0]; % group 3 1st bar
...
b(4).CData(1,:) = [0.9290 0.6940 0.1250]; % group 1 4th bar
b(4).CData(2,:) = [0.9290 0.6940 0.1250]; % group 2 4th bar
b(4).CData(3,:) = [0.9290 0.6940 0.1250]; % group 3 4th bar
